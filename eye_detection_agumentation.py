#eye extraction with rotation agumentation
import dlib
            ...
            try:
                ...
                for i in range(3):
                    try:
                        ...
                        eye = funcs.subimage(image=img,center=(mid_x,mid_y),theta=rnd,width=recYSize,height=recXSize)
                        plt.figure()
                        ...
                        cv2.imwrite(io.getPlotsOutputPath()+dir+'/' + rnd.__str__()+'_'+subject,
                                    image_to_write)
                        df.loc[f, 'eye_detected'] = df.loc[f, 'eye_detected']+1
